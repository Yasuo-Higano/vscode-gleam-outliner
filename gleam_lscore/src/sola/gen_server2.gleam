...
pub type ServerResponse(result, env) {
    Reply(result: result, env: env)
    Noreply(env)
}
...
pub fn start_link(def:GenServer(inittype,msgtype,state,result),initparam:inittype) -> Result(Pid,err) {
    let atom = string_to_atom("sola@gen_server2") 
    start_link_(atom,#(def,initparam),[])
}

//pub fn init( initarg ) -> Result(env, err) {
pub fn init( initarg ) {
    let #(def, initparam) = initarg
    Ok( Env(def:def, state:def.init(initparam)) )
}

pub fn handle_call(msg:msgtype, from:Pid, env:Env(inittype,msgtype,state,result)) -> ServerResponse(result,Env(inittype,msgtype,state,result)) {
    let res = env.def.call(msg,env.state)
    Reply(res.result, Env(..env, state:res.state))
}

pub fn handle_cast(msg:msgtype, env:Env(inittype,msgtype,state,result)) -> ServerResponse(result,Env(inittype,msgtype,state,result)) {
    let res = env.def.cast(msg,env.state)
    Noreply(Env(..env, state:res.state))
}

pub fn handle_info(msg:msgtype, env:Env(inittype,msgtype,state,result)) -> ServerResponse(result,Env(inittype,msgtype,state,result)) {
